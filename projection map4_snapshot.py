...
for j in (0,3) :
    ...
    for k,tidx in enumerate(snap):#range(250, 499):  # time step 251, 331, 411, 501
        snc = sn[(tidx-2 <= sn['time']) & (sn['time'] < tidx+1)]
        #runaway=sn['mass'] == 0.0
        #snc = snc[~runaway]
        snc_x = snc['x1']/4.+ 128
        snc_z = snc['x3']/4 + 896
        ...
        vel_z = vel_z[:,sl,:]
        temp = temp[:, sl, :]
        vel_z_dif = copy.copy(vel_z)
        vel_z[temp>20000]=np.nan
        vel_z_dif[temp<20000]=np.nan
        ...
        plt.subplot(1,6,3)
        im3=plt.imshow(vel_z,origin='lower',cmap='coolwarm',aspect='auto',vmin=-100,vmax=100) #,vmin=-200,vmax=200)# ,norm=colors.SymLogNorm(linthresh=100,vmin=-200,vmax=200)
        #print 'solar',d2.max(), d2.min()
        #plt.title(r'Vel_z [km/s]')
        if resol=='8pc':
            plt.xticks([0,64,128],['-0.5','0','0.5'])
        ...
        plt.subplot(1,6,4)
        im4=plt.imshow(vel_z_dif,origin='lower',cmap='coolwarm',aspect='auto',vmin=-500,vmax=500) #,vmin=-200,vmax=200)# ,norm=colors.SymLogNorm(linthresh=100,vmin=-200,vmax=200)
        #print 'solar',d2.max(), d2.min()
        #plt.title(r'Vel_z [km/s]')
        if resol=='8pc':
            plt.xticks([0,64,128],['-0.5','0','0.5'])
        ...
        plt.subplot(1,6,5)
        im5=plt.imshow(temp,origin='lower',cmap='plasma',norm=LogNorm(),aspect='auto',vmin=5e2,vmax=1e8)#,vmin=1e3,vmax=1e7) #copper 4pc, slice gnuplot2_r
        #print 'solar',d2.max(), d2.min()
        #plt.title(r'Vel_z [km/s]')
        if resol=='8pc':
            plt.xticks([0,64,128],['-0.5','0','0.5'])
        ...
        plt.subplot(1,6,6)
        im6=plt.scatter(snc_x,snc_z,c='k',s=1)
        #print snc_x, snc_z
        #print 'solar',d2.max(), d2.min()
        #plt.title(r'Vel_z [km/s]')
        if resol=='8pc':
        ...
        print tidx
        cbar_1=plt.gcf().add_axes([0.77, 0.80, 0.02, 0.15])
        cbar_2=plt.gcf().add_axes([0.77, 0.63, 0.02, 0.15])
        cbar_3=plt.gcf().add_axes([0.77, 0.46, 0.02, 0.15])
        cbar_4 = plt.gcf().add_axes([0.77, 0.29, 0.02, 0.15])
        cbar_5 = plt.gcf().add_axes([0.77, 0.12, 0.02, 0.15])
        ...
        cbar4 = plt.colorbar(im4, cax=cbar_4, label=r'V$_z$ [km/s]')
        cbar5 = plt.colorbar(im5, cax=cbar_5, label=r'Temperature [K]')

        plt.subplots_adjust(bottom=0.1, top=0.99, wspace=.25,right=0.76,left=0.1,hspace=0.11)
        #plt.tight_layout()
        plt.savefig('D:/yeongu/plots/map_new/newmap_slice_y0_temcut_%s_%s.png' % (simid,tidx),dpi=200)
        plt.close()
        #plt.show()
    #plt.show()
    #plt.draw()
