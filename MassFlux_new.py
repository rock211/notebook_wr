...
for j in (0,1,2): #range(1,7) :
    ...
    for tidx in range(250, stop):  # time step 251, 331, 411, 501
        ...
        if j==0:
            ...
            d_d_hi = d_down[temp_down > 20000];  v_d_hi = vel_z_down[temp_down > 20000]

            #m_d_cuw = np.sum(np.sum(d_d_cuw[v_d_cuw < -67.1] * v_d_cuw[v_d_cuw < -67.1] / unit['time'].value)) * 512 / (8 * 1024 * 1024) # velocity cut
            #m_d_hi = np.sum(np.sum(d_d_hi[v_d_hi < -67.1] * v_d_hi[v_d_hi < -67.1] / unit['time'].value)) * 512 / (8 * 1024 * 1024) # velocity cut
    ...
    plt.text(249,hh[j]*0.9,'%s' % labell[j],fontsize=20)
    icm_c = 'green'
    if j == 0:
...
#plt.axes().tick_params(which='major', direction='in',labelsize=22)
#plt.tight_layout(pad=0.3)
plt.savefig('D:/yeongu/plots/massflux_No12_new3.png',dpi=400) # ,transparent=True
plt.savefig('D:/yeongu/plots/massflux_No12_new3.eps',format='eps',dpi=400)
plt.show()
#plt.close()
